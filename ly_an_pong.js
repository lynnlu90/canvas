...
(lib.player = function(mode,startPosition,loop) {
	...
	this.shape.setTransform(0,0,0.93,1);

	this.timeline.addTween(cjs.Tween.get(this.shape).wait(1));

}).prototype = p = new cjs.MovieClip();
p.nominalBounds = new cjs.Rectangle(-40,-7.5,80,15);
...
(lib.ball = function(mode,startPosition,loop) {
	...
	// Layer 1
	this.shape = new cjs.Shape();
	this.shape.graphics.f("#FFFFFF").s().p("AiBCCQg2g2AAhMQAAhLA2g3QA2g2BLAAQBMAAA2A2QA2A3ABBLQgBBMg2A2Qg2A2hMAAQhLAAg2g2g");
	this.shape.setTransform(0,0,0.811,0.811);
...
(lib.ly_an_pong = function(mode,startPosition,loop) {
	...
	this.frame_0 = function() {
		...
		function moveBall() {
			ball.x += ball.xdir;
			ball.y += ball.ydir;
			if (ball.y > height - 22.5) {
				if ((ball.x >= player.x && ball.x - player.x <= 46) || (ball.x < player.x && player.x - ball.x < 46)) {
